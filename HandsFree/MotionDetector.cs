    ...
    class MotionDetector
    {
    	...
    	public static bool Compute(ref Image<Bgr, byte> imageSource, ref Image<Gray, byte> imageResult)
    	{
    		...
    		if (imageBackground != null)
    		{
                ...
                //find center of motion by finding the median of X and Y positions of pixels brighter than base threshold
                List<int> xArray = new List<int>(), yArray = new List<int>();
                ...
                if (candidates.Count > 0)
                {
                    ...
                    //smooth image to cancel noise, and get count of pixels above threshold
                    imageProcessed._ThresholdBinary(new Gray(baseThreshold), white);
                    ...
                    for (int i = 0; i < imageWidth; i++)
                        for (int j = 0; j < imageHeight; j++)
                            ...
                            {
                                imageSmoothed.Data[j, i, 0] = 255;
                                pixCount++;
                            }
                            else
                                imageSmoothed.Data[j, i, 0] = 0;

                    //heuristics to recognize motion as raising hand
                        ...
                        for (int i = 0; i < imageWidth; i++)
                            for (int j = 0; j < imageHeight; j++)
                                if (imageProcessed.Data[j, i, 0] == 255)
                                    if (i < imageWidth / 2)
                                    {
                                        xWeightedLeft += 1d - (i / (imageWidth / 2d));
                                        xWeightedCount++;
                                    }
                                    else
                                    {
                                        xWeightedRight += 1d - (imageWidth - i) / (imageWidth / 2d);
                                        xWeightedCount++;
                                    }
                    ...
                    else if (!gestureActive && gestureActiveOld)
                    {
                        ...
                        if (motionTimerCount >= 1 && motionTimerCount < 50 && size > 0.3d && motionWidth < 15)
                        {
                            ...
                            //obtain preliminary hand silhouette by computing frame difference between images before and after motion, then thresholding image by base threshold
                            byte baseThreshold2 = 0;
        ...
        private static void getFrameDifference(ref Image<Bgr, byte> foregoundImage, ref Image<Bgr, byte> backgroundImage, ref Image<Gray, byte> imageResult, ref byte baseThreshold)
        {
            ...
            //get frame difference of foreground and background frames, use the maximum value between differences in R, G, and B channels
            for (int i = 0; i < imageWidth; i++)
                for (int j = 0; j < imageHeight; j++)
                {
            ...
            //find out base threshold by surveying a 20*20 block on the top-right corner
            for (int i = imageWidth - 20; i < imageWidth; i++)
                for (int j = imageHeight / 3; j >= 0; j--)
                    if (imageResult.Data[j, i, 0] > baseThreshold)
                        baseThreshold = imageResult.Data[j, i, 0];
        }
